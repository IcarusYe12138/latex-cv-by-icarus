...
預設主題色是品牌色 `#2F2FE4`，但這個 Skill 的定位是「自由」。如果想換成自己的履歷主題色：

1. 到 <https://colorhunt.co/> 瀏覽並選一組 4 色配色
2. 選 1 個作為主色，再選 1 個深色作為正文
3. 在你要用的模板裡把這一行替換掉：
...
- 複製 HEX 值時去掉 `#`
- A / P × 中文 / 英文四份模板同步替換，保持一致
- 建議只改主色一項，背景、連結、灰階保持預設，避免版面過花

完整調參階梯（邊距、間距、行距、字號）見 [references/版式与调参.md](references/版式与调参.md)。
