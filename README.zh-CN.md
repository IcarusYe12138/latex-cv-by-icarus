...
默认主题色是品牌色 `#2F2FE4`，但这个 Skill 的定位是“自由”。如果想换成自己的简历主题色：

1. 到 <https://colorhunt.co/> 浏览并选一组 4 色配色
2. 选 1 个作为主色，再选 1 个深色作为正文
3. 在你要用的模板里把这一行替换掉：
...
- 复制 HEX 值时去掉 `#`
- A / P × 中文 / 英文四份模板同步替换，保持一致
- 建议只改主色一项，背景、链接、灰阶保持默认，避免版面过花

完整调参阶梯（边距、间距、行距、字号）见 [references/版式与调参.md](references/版式与调参.md)。
